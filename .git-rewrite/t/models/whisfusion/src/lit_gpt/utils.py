...
class NotYetLoadedTensor:
    ...
    def __getattr__(self, name):
        # properties
        # Handle tensor properties dynamically
        if name in {
            "dtype",
            "grad",
            "grad_fn",
...
class IncrementalPyTorchPickler(pickle.Pickler):
    ...
    def persistent_id(self, obj):
        # Note: PyTorch's persistent_id returns tuples instead of strings
        # This works with binary protocol but diverges from Python docs
        # References:
        ...
        if isinstance(obj, torch.storage.TypedStorage) or torch.is_storage(obj):
            if isinstance(obj, torch.storage.TypedStorage):
                # Note: This case handles TypedStorage for backward compatibility
                storage = obj._untyped_storage
