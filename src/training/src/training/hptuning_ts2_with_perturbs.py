...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--trainer_script", type=str,
    ...
    p.add_argument("--resume_existing", action="store_true",
...
def default_trials() -> list[TrialSpec]:
    return [
        TrialSpec("T02_perturb_p10_m70_100", 0.7, 1.0, 0.1, True),
        TrialSpec("T03_perturb_p30_m70_100", 0.7, 1.0, 0.3, True),
        TrialSpec("T04_perturb_p50_m70_100", 0.7, 1.0, 0.5, True),
        TrialSpec("T03_perturb_p30_m10_100", 0.1, 1.0, 0.3, True),
        TrialSpec("T06_perturb_p30_m70_100_no_perturb_loss", 0.8, 1.0, 0.2, False),
    ]
...
def init_results_csv(path: Path) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "trial_name", "status", "selection_metric", "best_val_loss", "best_val_wer", "best_val_cer",
            "best_epoch", "stopped_epoch", "min_mask_ratio", "max_mask_ratio", "use_phoneme_perturber",
            "perturb_prob", "include_perturb_in_loss", "model_best_path", "model_best_ema_path",
            "resolved_out_dir"
        ])
        writer.writeheader()
...
def build_command(args: argparse.Namespace, trial: TrialSpec, trial_dir: Path) -> list[str]:
    cmd = [
        "fabric", "run", args.trainer_script,
        "--strategy=ddp",
    ...
    if trial.use_phoneme_perturber:
        cmd.append("--use_phoneme_perturber")
        cmd.extend(["--perturb_prob", str(trial.perturb_prob)])
        cmd.extend(["--perturber_k", str(trial.perturber_k)])
...
def main() -> int:
    ...
    with log_path.open("a", encoding="utf-8") as log:
        log.write(f"Starting tuner in {root}\n")
        log.write("Edit default_trials() in this file to change the hardcoded search set.\n")
        for trial in trials:
            ...
            if prior.get("status") == "COMPLETED" and metrics_path.exists():
                log.write(f"Skipping completed trial {trial.trial_name}\n")
                continue
            ...
            log.write(f"Running {trial.trial_name}: {' '.join(cmd)}\n")
            log.flush()
            ...
            if result.returncode != 0:
                ...
                write_json(state_path, state)
                log.write(f"Trial {trial.trial_name} failed with return code {result.returncode}\n")
            ...
            if not metrics_path.exists():
                ...
                write_json(state_path, state)
                log.write(f"Trial {trial.trial_name} missing final_metrics.json\n")
                return 2
            ...
            append_result(results_csv, {
                ...
                "use_phoneme_perturber": metrics.get("used_perturbation_train"),
            ...
            log.write(f"Completed {trial.trial_name}\n")
            log.flush()
        ...
        if not completed:
            log.write("No completed trials found; nothing to promote.\n")
            return 3
        ...
        if promote_src.exists():
            ...
            log.write(f"Promoted best model from {promote_src}\n")
        else:
            log.write("Best trial completed but no model_best.pt found to promote.\n")

        log.write(f"Best trial: {best_trial_name}\n")
        return 0
