...
def main(fabric: L.Fabric, args, out_dir: Path):
    ...
    for epoch in range(state["epoch"], args.epochs):
        ...
        if fabric.global_rank == 0:
            ...
            if perturber is not None and epoch_perturb["visible_tokens_total"] > 0:
                epoch_visible_rate = (
                    epoch_perturb["visible_perturbed_tokens_total"] / epoch_perturb["visible_tokens_total"]
                )
                epoch_seq_hit_rate = (
                    epoch_perturb["changed_sequences"] / epoch_perturb["total_sequences"]
                    if epoch_perturb["total_sequences"] > 0 else 0.0
                )
                fabric.print(
                    f"[Epoch {epoch+1}] visible perturbed token rate={epoch_visible_rate:.2%} "
                    f"| seq hit rate={epoch_seq_hit_rate:.2%} "
                    f"| changed_words={epoch_perturb['changed_words']}/{epoch_perturb['attempted_words']}"
                )
