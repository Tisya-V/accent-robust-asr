                ...
                metrics.update(
                    {
                        ...
                        "total_tokens": self.history_lengths[-1] * world_size * samples / self.iter,
                    }
                )
...
class SpeedMonitorFabric(SpeedMonitorBase):
    def __init__(self, fabric: Fabric, *args: Any, **kwargs: Any) -> None:
        # Note: Precision plugin compatibility may require adjustment
...
def estimate_flops(model: torch.nn.Module) -> int:
