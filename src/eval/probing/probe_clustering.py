"""
probe_clustering.py
Clustering metrics + UMAP/t-SNE visualisations for Whisper encoder representations.
...
Computes per layer per model:
  ...
  - UMAP + t-SNE 2D projections saved as PNG
...
Output:
    ...
    results/clustering/{model_key}/tsne_*.png
"""
...
# ---------------------------------------------------------------------------
# 2-D projection plotting — UMAP or t-SNE
# ---------------------------------------------------------------------------

# t-SNE max sensible points (slow beyond this)
_TSNE_MAX = 2_000
# UMAP can handle more
_UMAP_MAX = 3_000
...
def _project(
    ...
    if method == "umap":
        ...
        return TSNE(
            n_components = 2,
            perplexity   = min(perplexity, len(X_scaled) // 4),
            init         = "pca",
            random_state = 42,
            n_jobs       = -1,
        ).fit_transform(X_scaled)
...
def make_projection_plot(
    ...
    max_pts = _TSNE_MAX if method == "tsne" else _UMAP_MAX
    if len(X) > max_pts:
        idx    = np.random.RandomState(42).choice(len(X), max_pts, replace=False)
        X      = X[idx]
        labels = labels[idx]

    X_scaled  = StandardScaler().fit_transform(X)
    embedding = _project(X_scaled, method,
...
# ---------------------------------------------------------------------------
# Helper: run both projections for a given (X, labels, label_names) combo
# ---------------------------------------------------------------------------

def _both_projections(
    X:           np.ndarray,
    labels:      np.ndarray,
    label_names: list[str],
    title_base:  str,
    out_stem:    str,          # full path stem, e.g. ".../umap_baseline_layer4_l1"
                               # method prefix is already in the stem
    umap_dir:    Path,
    **proj_kwargs,
) -> None:
    """Emit one UMAP PNG and one t-SNE PNG for the same data."""
    for method in ("umap", "tsne"):
        # replace leading "umap_" prefix in stem with correct method prefix
        stem     = Path(out_stem).name.replace("umap_", f"{method}_", 1)
        out_path = str(umap_dir / stem)
        make_projection_plot(
            X, labels, label_names,
            title    = f"{title_base} [{method.upper()}]",
            out_path = out_path,
            method   = method,
            **proj_kwargs,
        )
...
def cluster_model(
    ...
    for layer_idx in layer_indices:
        ...
        _both_projections(
            X, l1_ids, L1_GROUPS,
            title_base = f"{model_key} | Layer {layer_idx} | by L1",
            out_stem   = str(umap_dir / f"{base}_l1"),
            umap_dir   = umap_dir,
        )

        _both_projections(
            X, phone_ids, ARPABET_VOCAB,
            title_base = f"{model_key} | Layer {layer_idx} | by phoneme",
            out_stem   = str(umap_dir / f"{base}_phoneme"),
            umap_dir   = umap_dir,
        )
        ...
        if mask_f.sum() > 20:
            ...
            _both_projections(
                X[mask_f], pid_remapped, focus_labels,
                title_base = f"{model_key} | Layer {layer_idx} | focus phones",
                out_stem   = str(umap_dir / f"{base}_focus_phones"),
                umap_dir   = umap_dir,
            )

            _both_projections(
                X[mask_f], l1_ids[mask_f], L1_GROUPS,
                title_base = f"{model_key} | Layer {layer_idx} | focus phones by L1",
                out_stem   = str(umap_dir / f"{base}_focus_phones_l1"),
                umap_dir   = umap_dir,
            )
...
def main():
    ...
    p = argparse.ArgumentParser(description="Clustering metrics + UMAP/t-SNE for Whisper encoder")
    ...
    p.add_argument("--layers",
                   default=",".join(str(i) for i in range(WHISPER_N_ENCODER_LAYERS + 1)),
                   help="Comma-separated layer indices (default: all 13)")
    p.add_argument("--umap_layers",          default="0,4,8,12",
                   help="Subset of layers to generate projection plots for (expensive)")
    p.add_argument("--focus_phones",         default="TH,DH,V,F,S,Z,T,D",
                   help="Phones to highlight in focus-phone plots")
